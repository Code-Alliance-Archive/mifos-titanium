...
// in simulator we redefine to format for mifos Developer console
...
{\
	NSString * exceptionName = [@"org.mifos." stringByAppendingString:NSStringFromClass([self class])];\
...
+ (void) throwException:(NSString *) reason subreason:(NSString*)subreason location:(NSString *)location\
{\
	NSString * exceptionName = @"org.mifos";\
	NSString * message = [NSString stringWithFormat:@"%@. %@ %@",reason,(subreason!=nil?subreason:@""),(location!=nil?location:@"")];\
	TiExceptionThrowWithNameAndReason(exceptionName,message);\
}\
...
enum {
	UImifosNativeItemNone = -1, 
	UImifosNativeItemSpinner = -2,
	UImifosNativeItemProgressBar = -3,
	
	UImifosNativeItemSlider = -4,
	UImifosNativeItemSwitch = -5,
	UImifosNativeItemMultiButton = -6,
	UImifosNativeItemSegmented = -7,
	
	UImifosNativeItemTextView = -8,
	UImifosNativeItemTextField = -9,
	UImifosNativeItemSearchBar = -10,
	
	UImifosNativeItemPicker = -11,
	UImifosNativeItemDatePicker = -12,
	
	UImifosNativeItemInfoLight = -13,
	UImifosNativeItemInfoDark = -14,
	
	UImifosNativeItemDisclosure = -15,
	
	UImifosNativeItemContactAdd = -16
};
...
/**
 *	TiThreadPerformOnMainThread should replace all mifos instances of
